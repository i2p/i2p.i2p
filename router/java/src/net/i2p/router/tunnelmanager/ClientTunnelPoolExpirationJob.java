...
class ClientTunnelPoolExpirationJob extends JobImpl {
    private final static Log _log = new Log(ClientTunnelPoolExpirationJob.class);
    ...
    public ClientTunnelPoolExpirationJob(ClientTunnelPool pool, TunnelPool tunnelPool) {
	super();
	_pool = pool;
	...
	if (_pool.isStopped()) {
		...
		_log.info("Client " + _pool.getDestination().calculateHash() + " is stopped, but they still have some tunnels, so don't stop expiring");
	    }
	}
	...
	for (Iterator iter = _pool.getInactiveInboundTunnelIds().iterator(); iter.hasNext(); ) {
		...
		if (info.getSettings().getExpiration() < expire) {
		    _log.info("Expiring inactive tunnel " + id + " [" + new Date(info.getSettings().getExpiration()) + "]");
