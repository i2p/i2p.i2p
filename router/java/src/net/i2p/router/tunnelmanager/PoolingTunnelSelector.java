...
class PoolingTunnelSelector {
    private final static Log _log = new Log(PoolingTunnelSelector.class);
    /** don't use a tunnel thats about to expire */
    private static long POOL_USE_SAFETY_MARGIN = 10*1000;
    
    public static List selectOutboundTunnelIds(TunnelPool pool, TunnelSelectionCriteria criteria) {
    ...
    public static List selectInboundTunnelIds(TunnelPool pool, TunnelSelectionCriteria criteria) {
    ...
    private final static List randomize(TunnelPool pool, List tunnelIds) {
	...
	Collections.shuffle(rv, RandomSource.getInstance());
	return rv;
    }
    
    private final static boolean isAlmostExpired(TunnelPool pool, TunnelId id, long safetyMargin) {
	...
	if (info.getSettings().getExpiration() - safetyMargin <= Clock.getInstance().now()) {
