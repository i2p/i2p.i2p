...
class ClientTunnelPool {
    ...
    public ClientTunnelPool(Destination dest, ClientTunnelSettings settings, TunnelPool pool) {
	_dest = dest;
	_settings = settings;
	_pool = pool;
	...
	if (info != null) {
	    MessageHistory.getInstance().tunnelJoined("inactive inbound", info);
	...
	for (Iterator iter = activeTunnels.iterator(); iter.hasNext(); ) {
	    ...
	    MessageHistory.getInstance().tunnelJoined("active inbound", info);
	...
	MessageHistory.getInstance().tunnelJoined("active inbound", tunnel);
	synchronized (_inboundTunnels) {
