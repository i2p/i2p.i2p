...
class PHTTPSender {
    ...
    public PHTTPSender(PHTTPTransport transport) {
	...
	if (sendURL == null) {
	    ...
	    BandwidthLimiter.getInstance().delayOutbound(msg.getTarget().getIdentity(), data.length+512); // HTTP overhead
	...
	try {
	    ...
	    do {
		BandwidthLimiter.getInstance().delayOutbound(msg.getTarget().getIdentity(), 512); // HTTP overhead
		BandwidthLimiter.getInstance().delayInbound(msg.getTarget().getIdentity(), 512); // HTTP overhead
	...
	try {
	    ...
	    buf.append(PARAM_SEND_TIMEOUTMS).append('=').append(msg.getExpiration() - Clock.getInstance().now()).append('&');
	    buf.append(PARAM_SEND_DATA_LENGTH).append('=').append(msg.getMessageSize()).append('&');
	    buf.append(PARAM_SEND_TIME).append('=').append(Clock.getInstance().now()).append('&').append('\n');
