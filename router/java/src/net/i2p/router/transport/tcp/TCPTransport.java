...
public class TCPTransport extends TransportImpl {
    private final static Log _log = new Log(TCPTransport.class);
    ...
    static {
	StatManager.getInstance().createFrequencyStat("tcp.attemptFailureFrequency", "How often do we attempt to contact someone, and fail?", "TCP Transport", new long[] { 60*1000l, 60*60*1000l, 24*60*60*1000l });
	StatManager.getInstance().createFrequencyStat("tcp.attemptSuccessFrequency", "How often do we attempt to contact someone, and succeed?", "TCP Transport", new long[] { 60*1000l, 60*60*1000l, 24*60*60*1000l });
	StatManager.getInstance().createFrequencyStat("tcp.acceptFailureFrequency", "How often do we reject someone who contacts us?", "TCP Transport", new long[] { 60*1000l, 60*60*1000l, 24*60*60*1000l });
	StatManager.getInstance().createFrequencyStat("tcp.acceptSuccessFrequency", "How often do we accept someone who contacts us?", "TCP Transport", new long[] { 60*1000l, 60*60*1000l, 24*60*60*1000l });
	StatManager.getInstance().createRateStat("tcp.connectionLifetime", "How long do connections last (measured when they close)?", "TCP Transport", new long[] { 60*1000l, 60*60*1000l, 24*60*60*1000l });
    }
    ...
    public TCPTransport(RouterIdentity myIdentity, SigningPrivateKey signingKey, RouterAddress address) {
	super();
	...
	JobQueue.getInstance().addJob(new JobImpl() {
	    public void runJob() {
	...
	try {
		...
		startEstablish = Clock.getInstance().now();
		if (getStyle().equals(addr.getTransportStyle())) {
		    ...
		    socketCreated = Clock.getInstance().now();
		    if (s == null) {
			if (_log.shouldLog(Log.WARN))
			    _log.warn("Unable to establish a socket in time to " + addr);
			ProfileManager.getInstance().commErrorOccurred(target.getIdentity().getHash());
			return false;
		    }
		    if (_log.shouldLog(Log.DEBUG))
			_log.debug("Socket created");
		    if (s != null) {
			TCPConnection con = new RestrictiveTCPConnection(s, _myIdentity, _signingKey, true);
			conCreated = Clock.getInstance().now();
			if (_log.shouldLog(Log.DEBUG))
			    _log.debug("TCPConnection created");
			boolean established = handleConnection(con, target);
			conEstablished = Clock.getInstance().now();
			if (_log.shouldLog(Log.DEBUG))
	...
	bid.setExpiration(new Date(Clock.getInstance().now()+1000*60)); // 1 minute
	...
	String str = Router.getInstance().getConfigSetting(PROP_ESTABLISHERS);
	if (str != null) {
	    try {
    ...
    public RouterIdentity getMyIdentity() { return _myIdentity; }
	...
	synchronized (_connections) {
		...
		StatManager.getInstance().addRateData("tcp.connectionLifetime", lifetime, 0);
	    }
	...
	//    ProfileManager.getInstance().commErrorOccurred(con.getRemoteRouterIdentity().getHash());
    }
	...
	long afterEstablish = Clock.getInstance().now();
	long startRunning = 0;
	
	if (ident == null) {
	    StatManager.getInstance().updateFrequency("tcp.acceptFailureFrequency");
	    con.closeConnection();
	...
	if (target != null) {
		...
		NetworkDatabaseFacade.getInstance().fail(target.getIdentity().getHash());
		return false;
	    } else {
		if (_log.shouldLog(Log.DEBUG))
	...
	if (ident != null) {
	    ...
	    Shitlist.getInstance().unshitlistRouter(ident.getHash());
	    con.runConnection();
	    startRunning = Clock.getInstance().now();
		...
		for (Iterator iter = toClose.iterator(); iter.hasNext(); ) {
		    ...
		    StatManager.getInstance().addRateData("tcp.connectionLifetime", oldCon.getLifetime(), 0);
		}
	    }
	    long done = Clock.getInstance().now();
	...
	StatManager.getInstance().updateFrequency("tcp.acceptSuccessFrequency");
	return true;
    }
	...
	public void run() {
		...
		try {
		    ...
		    long conFetched = Clock.getInstance().now();
		    long sentPending = 0;
			...
			sentPending = Clock.getInstance().now();
		    } else {
			boolean established = establishConnection(pending.getPeerInfo());
			establishedCon = Clock.getInstance().now();
			if (established) {
			    StatManager.getInstance().updateFrequency("tcp.attemptSuccessFrequency");
			    if (_log.shouldLog(Log.DEBUG))
	    			_log.debug("Connection established");
			    con = getConnection(pending.getPeerInfo().getIdentity());
			    refetchedCon = Clock.getInstance().now();
			    if (con == null) {
				if (_log.shouldLog(Log.ERROR))
				    _log.error("Connection established but we can't find the connection? wtf!  peer = " + pending.getPeer());
			    } else {
				Shitlist.getInstance().unshitlistRouter(pending.getPeer());
				sendPending(con, pending);
				sentRefetched = Clock.getInstance().now();
			    }
			} else {
			    StatManager.getInstance().updateFrequency("tcp.attemptFailureFrequency");
			    if (_log.shouldLog(Log.INFO))
	    			_log.info("Unable to establish a connection to " + pending.getPeer());
			    failPending(pending);
			    Shitlist.getInstance().shitlistRouter(pending.getPeer());
			    //ProfileManager.getInstance().commErrorOccurred(pending.getPeer());
			    failedPending = Clock.getInstance().now();
			}
		    }
