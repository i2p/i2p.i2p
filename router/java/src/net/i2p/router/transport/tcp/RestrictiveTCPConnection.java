...
class RestrictiveTCPConnection extends TCPConnection {
    private final static Log _log = new Log(RestrictiveTCPConnection.class);
    
    public RestrictiveTCPConnection(Socket s, RouterIdentity myIdent, SigningPrivateKey signingKey, boolean locallyInitiated) {
	super(s, myIdent, signingKey, locallyInitiated);
    }
    ...
    static {
    	StatManager.getInstance().createRateStat("tcp.establishConnectionTime", "How long does it take for us to successfully establish a connection (either locally or remotely initiated)?", "TCP Transport", new long[] { 60*1000l, 60*60*1000l, 24*60*60*1000l });
    }
	...
	byte encr[] = AESEngine.getInstance().safeEncrypt(baos.toByteArray(),  _key, _iv, 16);
	...
	if (us != them) {
	    ...
	    if (them > us)
		Router.getInstance().setHigherVersionSeen(true);
	    return false;
	} else {
	...
	byte encr[] = AESEngine.getInstance().safeEncrypt(baos.toByteArray(), _key, _iv, 256);
	DataHelper.writeLong(_out, 2, encr.length);
	_out.write(encr);
	...
	try {
		...
		NetworkDatabaseFacade.getInstance().fail(_remoteIdentity.getHash());
		// but we make sure that we don't try to talk to them soon even if we get a new ref
		Shitlist.getInstance().shitlistRouter(_remoteIdentity.getHash());
	    ...
	    if (success > 0)
		StatManager.getInstance().addRateData("tcp.establishConnectionTime", success-start, success-start);
	}
    }
}
