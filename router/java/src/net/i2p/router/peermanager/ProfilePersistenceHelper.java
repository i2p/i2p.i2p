...
class ProfilePersistenceHelper {
    private final static Log _log = new Log(ProfilePersistenceHelper.class);
    private final static ProfilePersistenceHelper _instance = new ProfilePersistenceHelper();
    public final static ProfilePersistenceHelper getInstance() { return _instance; }
    ...
    private ProfilePersistenceHelper() {
	...
	long delay = Clock.getInstance().now() - before;
	...
	String groups = null;
	if (ProfileOrganizer.getInstance().isFailing(profile.getPeer())) {
	    groups = "failing";
	} else if (!ProfileOrganizer.getInstance().isReliable(profile.getPeer())) {
	    groups = "not failing";
	} else {
	    if (ProfileOrganizer.getInstance().isFastAndReliable(profile.getPeer())) 
		groups = "fast and reliable";
	    else
		groups = "reliable";
	    
	    if (ProfileOrganizer.getInstance().isWellIntegrated(profile.getPeer()))
	...
	long duration = Clock.getInstance().now() - start;
	if (_log.shouldLog(Log.DEBUG))
	...
	if (_profileDir == null) {
	    String dir = Router.getInstance().getConfigSetting(PROP_PEER_PROFILE_DIR);
	    if (dir == null) {
