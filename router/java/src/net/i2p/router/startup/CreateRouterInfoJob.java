...
public class CreateRouterInfoJob extends JobImpl {
    ...
    public CreateRouterInfoJob(Job next) {
	_next = next;
    }
    ...
    static RouterInfo createRouterInfo() {
	...
	try {
	    ...
	    info.setPublished(getCurrentPublishDate());
	    ...
	    KeyManager.getInstance().setSigningPublicKey(signingPubKey);
	    KeyManager.getInstance().setPrivateKey(privkey);
	    KeyManager.getInstance().setPublicKey(pubkey);
     ...
     *
     */
    static long getCurrentPublishDate() {
