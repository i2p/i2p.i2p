    ...
    public static String status(RouterContext ctx, HttpSession session) {
         ...
         if (lastUpdated > 0) {
             ...
             if (shouldShowNews(ctx)) {
                 buf.append(" <a id=\"toggle-news\" href=\"?news=0&amp;consoleNonce=").append(consoleNonce).append("\">")
                    .append(Messages.getString("Hide news", ctx));
             } else {
                 buf.append(" <a id=\"toggle-news\" href=\"?news=1&amp;consoleNonce=").append(consoleNonce).append("\">")
