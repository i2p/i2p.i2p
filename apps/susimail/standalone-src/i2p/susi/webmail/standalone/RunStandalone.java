package i2p.susi.webmail.standalone;
...
/**
 * @since 0.9.70 adapted from i2psnark standalone
 */
public class RunStandalone {
    
    private final JettyStart _jettyStart;
    private final I2PAppContext _context;
    private int _port = 8004;
    private String _host = "127.0.0.1";
    private static RunStandalone _instance;
    static final File APP_CONFIG_FILE = new File("susimail-appctx.config");
    private static final String PROP_DTG_ENABLED = "desktopgui.enabled";
    private static final String FULL_VERSION = WebMail.STANDALONE_VERSION;
    ...
    private RunStandalone(String args[]) throws Exception {
        ...
        File xml = new File(base, "jetty-susimail.xml");
    ...
    /**
     *  Usage: RunStandalone [host [port]] (but must match what's in the jetty-susimail.xml file)
     */
    ...
    public void start() {
        try {
            String url = "http://" + _host + ':' + _port + "/susimail/";
            System.out.println("Starting susimail " + FULL_VERSION + " at " + url);
            ...
            if (dtg != null)
                dtg.addMenu("Shutdown SusiMail", new StandaloneStopper(dtg));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static class StandaloneStopper implements MenuCallback {
        ...
        public void clicked(MenuHandle menu) {
            ...
            _ms.updateMenu("SusiMail shutting down", menu);
            Thread t = new I2PAppThread(new StopperThread(), "SusiMail Stopper", true);
            t.start();
        }
    }
