...
class LogRecordFormatter {
	...
	for (int i=0; i<format.length; ++i) {
	    switch ((int)format[i]) {
		case (int)LogManager.DATE:      buf.append(getWhen(rec)); break;
		case (int)LogManager.CLASS:     buf.append(getWhere(rec)); break;
		case (int)LogManager.THREAD:    buf.append(getThread(rec)); break;
		case (int)LogManager.PRIORITY:  buf.append(getPriority(rec)); break;
		case (int)LogManager.MESSAGE:   buf.append(getWhat(rec)); break;
		default:                        buf.append(format[i]); break;
	    }
	}
	buf.append(NL);
    ...
    private static String getWhen(LogRecord logRecord) { return LogManager.getInstance()._getDateFormat().format(new Date(logRecord.getDate())); }
    private static String getPriority(LogRecord rec) { return toString(Log.toLevelString(rec.getPriority()), MAX_PRIORITY_LENGTH); }
    private static String getWhat(LogRecord rec) { return rec.getMessage(); }
