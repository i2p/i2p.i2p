...
public class I2PThread extends Thread {    
	...
	try {
	    ...
	    if ( (t instanceof OutOfMemoryError) && (_lsnr != null) )
		_lsnr.outOfMemory((OutOfMemoryError)t);
    ...
    public static void setOOMEventListener(OOMEventListener lsnr) { _lsnr = lsnr; }
    public static OOMEventListener getOOMEventListener() { return _lsnr; }
	...
	I2PThread t = new I2PThread(new Runnable() {
	    public void run() { throw new NullPointerException("blah"); }
	});
	t.start();
	try { Thread.sleep(10000); } catch (Throwable tt) {}
    }
}
