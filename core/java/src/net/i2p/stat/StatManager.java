...
public class StatManager {
    ...
    public final static StatManager getInstance() { return _instance; }
    ...
    public FrequencyStat getFrequency(String name) { return (FrequencyStat)_frequencyStats.get(name); }
    public RateStat getRate(String name) { return (RateStat)_rateStats.get(name); }
    public Set getFrequencyNames() { return Collections.unmodifiableSet(new HashSet(_frequencyStats.keySet())); }
    ...
    public boolean isRate(String statName) { return _rateStats.containsKey(statName); }
    /** is the given stat a monitored frequency? */
    public boolean isFrequency(String statName) { return _frequencyStats.containsKey(statName); }
	...
	for (Iterator iter = _frequencyStats.values().iterator(); iter.hasNext();) {
	    ...
	    if (!groups.containsKey(stat.getGroupName()))
		groups.put(stat.getGroupName(), new TreeSet());
